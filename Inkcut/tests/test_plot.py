...
from inkcut_lib.plot import Plot
...
class TestPlot:
    ...
    def test_set_graphic(self):
        ...
        plot = Plot(90*12,90*12*4)
        plot.set_graphic(etree.tostring(etree.parse(os.path.join(TEST_FILE_DIR,"arrow.svg"))))
        f = open("tests/out/plot_%s.svg"%sys._getframe().f_code.co_name,"w")
    ...
    def test_set_graphic_2(self):
        ...
        plot = Plot(90*12,90*12*4)
        plot.set_graphic(etree.tostring(etree.parse(os.path.join(TEST_FILE_DIR,"arrow.svg"))))
        f = open("tests/out/plot_%s_1.svg"%sys._getframe().f_code.co_name,"w")
        f.write(plot.get_preview_xml())
        f.close()
        assert plot.get_width() == plot.graphic.get_width()
        assert round(plot.get_height(),10) == round(plot.graphic.get_height(),10) # why does it need rounding???
        plot.set_graphic(etree.tostring(etree.parse("fat-giraffes.svg")))
        f = open("tests/out/plot_%s_2.svg"%sys._getframe().f_code.co_name,"w")
        f.write(plot.get_preview_xml())
        f.close()
    ...
    def test_graphic_rotation(self):
        ...
        plot = Plot(90*12,90*12*4)
        plot.set_graphic(etree.parse(os.path.join(TEST_FILE_DIR,"arrow.svg")))
