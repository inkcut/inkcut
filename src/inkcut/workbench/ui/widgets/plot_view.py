...
from atom.api import (Callable,Tuple,Float, List, ContainerList, Bool,FloatRange, Unicode, Dict,Typed, ForwardTyped, observe)
...
class PlotView(Control):
    ...
    def _grid_changed(self,change):
        self.proxy.widget.showGrid(self.grid[0],self.grid[1],self.grid_alpha)
...
class QtPlotView(QtControl, ProxyPlotView):
    ...
    def set_data(self,data):
        ...
        if isinstance(data, (list,tuple)) and isinstance(data[0], GraphicsObject):
...
QT_FACTORIES['PlotView'] = plot_view_factory