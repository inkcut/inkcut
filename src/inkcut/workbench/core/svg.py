...
class QtSvgItem(QtGui.QPainterPath):
    ...
    _uuconv = {'in':90.0, 'pt':1.25, 'px':1, 'mm':3.5433070866, 'cm':35.433070866, 'm':3543.3070866,
          'km':3543307.0866, 'pc':15.0, 'yd':3240 , 'ft':1080}
    
    def __init__(self,e,*args,**kwargs):
        if not isinstance(e, etree._Element):
            raise TypeError("%s only works with etree Elements, given %s"%(self,type(e)))
        elif e.tag != self.tag:
            raise ValueError("%s only works with %s elements, given %s"%(self,self.tag,e.tag))
    ...
    def __imul__(self,m):
        """ Do in place multiplication by subtracting everything from itself then adding the multiplied values back. """
        tmp = self*m
        self -=self
        self +=tmp
        return self
    ...
    def parseTransform(self,e):
        """
        Based on simpletrasnform.py by from Jean-Francois Barraud, barraud@math.univ-lille1.fr
        ...
        m = re.match("(translate|scale|rotate|skewX|skewY|matrix)\s*\(([^)]*)\)\s*,?",trans)
        if m is None:
            return t
...
class QtSvgEllipse(QtSvgItem):
    ...
    def parse(self,e):
        c = QtCore.QPointF(*map(self.parseUnit,(e.attrib.get('cx',0),e.attrib.get('cy',0))))
        rx,ry = map(self.parseUnit,(e.attrib.get('rx',0),e.attrib.get('ry',0)))
        ...
        while True:
            ...
            if isCommand:
                if not lastCommand and token.upper() != 'M':
                    raise ValueError('Invalid path, must begin with moveto (M or m), given %s.'%lastCommand)    
                else:                
                    command = token
            else:
                #command was omited
            ...
            while numParams > 0:
                if needParam:
                    try: 
                        token, isCommand = lexer.next()
                        if isCommand:
                            raise ValueError('Invalid number of parameters for %s'%(command))
                    except StopIteration:
...
class QtSvgUse(QtSvgItem):
    ...
    def parseLink(self,e):
        ...
        if len(link)!=2:
            raise NotImplementedError("Cannot link to documents outside this document, given %s!"%("#".join(link),))
        link,id = link
...
class QtSvgDoc(QtSvgG):
    ...
    def __init__(self,e):
        ...
        """
        self.isParentSvg = not isinstance(e, etree._Element)
    ...
    def parseTransform(self,e):
        ...
        t =  QtGui.QTransform()
        if not self.isParentSvg: #  transforms don't apply to the root svg element
            x,y = map(self.parseUnit,(e.attrib.get('x',0),e.attrib.get('y',0)))
            t.translate(x,y)
            
        # TODO: Handle width/height and viewBox stuff
        #         else:
        #             w,h = map(self.parseUnit,(e.attrib.get('width',None),e.attrib.get('height',None)))
